...
function addResolutionScreenshot() {
    $('.js-screenshot-add').on('click', function(e) {
        e.preventDefault();
        var $target = $(this).closest(".add-resolutions__section")

        var screenshotBlock =  '<div class="col-12 col-md-9 col-lg-6 mt-4"><div class="screenshot-block screenshot-block--horizontal"><label class="input__wrapper mb-2"><div class="cover-image" id="image-holder2_0" style="display: none;"></div><div class="cover-image" id="image-holder2"><img src="/public/app_assets/img/photo-camera-1.svg" alt=""><input type="file" class="hide fileUpload2" name="screen3"></div></label><label class="input__wrapper mb-2"><textarea class="input height-115 ta-no-resize" name="screen3_description" maxlength="2000"> </textarea><p class="input__notification">Text is required</p></label></div></div>';

        $target.append($target.length ? screenshotBlock : '');
    });
}

function removeResolutionScreenshot() {
    $('.js-screenshot-remove').on('click', function(e) {
        e.preventDefault();

        var $target = $(this).closest(".add-resolutions__section");

        if ($target.length) {
            $target.find(".col-12.col-md-9.col-lg-6").last().remove();
        }
    });
}
...
function modalsShowSkills() {
    $(document).on('click', '.js-show-modal-skills', function(){
        var $self = $(this);
        var modalId = $self.data('modal-id');
        var list = $self.data('skills-list') && $self.data('skills-list').length ? $self.data('skills-list').split(', ') : '';
        // console.log(list);
        if (modalId && $('#' + modalId).length) {
            $('#' + modalId).modal({onShow: function () {
                $('#' + modalId).find('.dropdown').first().dropdown('set selected', list);
            }}).modal('show')
        }
    })
}
...
function initEvents() {
    ...
    if ($('body').hasClass('applying-for-job')) {
     ...
     //functions called for add-resolution / edit-resolution pages only
     if ($('body').hasClass('add-resolution')) {
        addResolutionScreenshot();
        removeResolutionScreenshot();
    }
